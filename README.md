...
└─ src-tauri/                  Backend (Rust)
   ...
   ├─ src/archives.rs          Which archive holds a mesh / texture path, for the `source_pak` labels
...
- `paks` plus `cache` (maclarian's `PakReaderCache`) are created once per game directory by `archives()`: opening an archive parses its whole file table, so the cache is sized for a full install (`CACHED_PAKS = 32`) and keeps every table it walked resident. Callers clone the pair back out and lock the cache only around a single read, so the state lock and the cache are never held at the same time. The list is maclarian's own directory scan (`find_pak_files`, recursive): the state only remembers what it is handed, the export pipeline only receives it, and an unopenable entry in it (a data partition, a `Localization` archive) is simply one more archive a read tries and skips. Answering which archive holds a path is `archives.rs`'s job (`PakIndex`)
...
- `fill_source_paks()` runs once, right after a database build: `VisualAsset::source_pak` and `TextureRef::source_pak` are declared by maclarian but never written by its parser, so the archive names are collected by listing the file tables with maclarian's own API (`PakOperations::list`) — the way its `extract_dds_textures` resolves a texture. Filling the fields once is what makes a name free everywhere it is shown (detail panel, `asset.json`), and the `PakIndex` behind it is dropped immediately afterwards: ~40 MB held for a session to answer lookups the fields already answer would be waste. It reports progress per archive, because it is the longer of the build's two phases and therefore owns the second half of the progress bar (`PARSE_SHARE`)
- Only meshes and textures are indexed (~330k paths among the 1.04M file-table entries of a full install): those are the only two file kinds whose archive is ever named. The pass measures ~1.8s unoptimized against ~0.7s optimized, i.e. longer than parsing the database itself, so it is what the build spends most of its time on. The first archive to list a path wins, i.e. the order `read_file` sweeps in, so a name can never contradict where the bytes came from
- Virtual textures carry an archive without any of this, because there it is decided per lookup by `GtpMatch::pak_path`
...
- Virtual textures are staged to a temp directory as `GTP` / `GTS`, extracted, and the temp directory is cleaned up afterwards. The extraction takes the page file's archive and its own path and nothing else: the sidecar is named after the tileset the page file carries (`..._3_<hash>.gtp` → `..._3.gts`), so the pair is settled by the name the file already has
...
- **The virtual texture archive is the one the match carries**: meshes and textures are scattered over the archives and are swept for, while every `.gtp` / `.gts` of a full install lives in `VirtualTextures.pak` — so the lookup is handed that single archive of the configured data directory (deliberately not maclarian's `virtual_textures_pak_path()`, which rebuilds the path from `bg3_data_path()`'s auto-detection and is empty for a hand-picked directory), and each `GtpMatch` records it in `pak_path`; the GTP read, the `.gts` lookup and the manifest all take the archive from that field instead of rebuilding it from a name, and the extractor is handed nothing but that archive plus the page file's path — the sidecar is named after the tileset the page file name carries (`..._3_<hash>.gtp` → `..._3.gts`), so nothing lists an archive to find it and nothing opens a GTS to guess
- **`<Name>_<n>.pak` data partitions are listed but never read**: they carry no LSPK header of their own, cannot be opened standalone, and are reachable through their main archive — the directory scan returns them like any other `.pak`, and every read skips them for free
...
└─ src-tauri/                  后端（Rust）
   ...
   ├─ src/archives.rs          某个网格 / 纹理路径属于哪个归档，供 `source_pak` 标注用
...
- `paks` 与 `cache`（maclarian 的 `PakReaderCache`）由 `archives()` 在每个游戏目录下只建一次：打开一个归档要解析整张文件表，因此缓存按完整安装的档案数配置（`CACHED_PAKS = 32`），使走过的表常驻；调用方克隆这对值后只在单次读取期间锁缓存，状态锁与缓存锁从不同时持有。列表本身来自 maclarian 自己的目录扫描（`find_pak_files`，递归）：状态只记住交回来的东西，导出流水线只接收它；其中打不开的条目（数据分片、`Localization` 归档）无非是读取时多试一次、然后被跳过；回答「某路径属于哪个归档」是 `archives.rs` 的职责（`PakIndex`）
...
- `fill_source_paks()` 在数据库构建结束后只跑一次：`VisualAsset::source_pak` / `TextureRef::source_pak` 虽由 maclarian 声明、其解析器却从不写入，因此归档名来自用官方 API 列文件表（`PakOperations::list`）——这正是其 `extract_dds_textures` 解析纹理的方式。一次填好字段，名字在展示它的每一处（详情面板、`asset.json`）都不再花代价，而背后的 `PakIndex` 随即释放：为回答已被字段回答过的查询而常驻约 40 MB 纯属浪费。它按归档逐个上报进度——这趟是构建两个阶段里更长的那个，因此进度条的后半段归它（`PARSE_SHARE`）
- 只索引网格与纹理（完整安装 1.04 M 条表项中约 330 k 条路径）：只有这两类文件会被标注归档。实测未优化约 1.8 s、优化后约 0.7 s，比解析数据库本身还久，因此构建的时间大头在这里。先列出该路径的归档胜出，即 `read_file` 的扫描顺序，故归档名绝不会与实际读取来源相矛盾
- 虚拟纹理不依赖这套机制：它的归档由每次查找的 `GtpMatch::pak_path` 当场决定
...
- 虚拟纹理经临时目录暂存 `GTP` / `GTS` 后提取，结束即清理。抽取只吃「页文件所在归档 + 页文件自身路径」两项：旁档就取页文件名里的那个 tileset 名（`..._3_<hash>.gtp` → `..._3.gts`），配对由文件已有的名字给出
...
- **虚拟纹理的归档由匹配结果携带**：网格与纹理分散在各档、需要扫描，而完整安装的所有 `.gtp` / `.gts` 都在 `VirtualTextures.pak` 里——因此查找时只把「当前配置的数据目录里的这一档」交给 maclarian（刻意不用 `virtual_textures_pak_path()`：它按 `bg3_data_path()` 自动探测重建路径，手工选目录时必为空），返回的每个 `GtpMatch` 把它记在 `pak_path`；GTP 读取、`.gts` 查找与 `asset.json` 都从该字段取归档，不再按归档名重建，且抽取器只接收「该归档 + 页文件路径」两项——旁档按页文件名里的 tileset 名取得（`..._3_<hash>.gtp` → `..._3.gts`），不再为找旁档列归档、也不再解析 GTS 去猜
- **`<Name>_<n>.pak` 数据分片会被列出但不读取**：它们没有独立 LSPK 头、无法单独打开，且已能通过主归档访问——目录扫描会像其他 `.pak` 一样返回它们，而每次读取都会免费跳过
