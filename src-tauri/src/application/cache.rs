...
//!
//! One file per game data directory, named after the directory itself (see `digest`), so several
//! installations can each keep their index and switching between them costs no scan. A file is only
//! used while it still matches the sources it was built from — the app version, the game's own paks and
...
/// One source file of a build, as it was when the build ran. The raw fields rather than a hash of
/// them: `DefaultHasher` promises nothing about its output across Rust versions, and a file name that
/// changes with the toolchain would silently orphan every cache on the disk
...
impl PakStamp {
    // The stamp of one file; `None` when it cannot be read, which is also how an absent pak shows up —
    // the two are indistinguishable in a set of stamps and both mean the file the build read is gone
...
/// stay in step, which is why each carries only what its own direction needs
#[derive(Deserialize)]
...
// The cache file, as written. Borrowed: the build still owns these values when the file is saved, and
// cloning a database of several hundred thousand entries just to write it out is not worth it
#[derive(Serialize)]
...
/// Write the index built for `game_path`, replacing whatever was there. Taken by reference and called
/// with the state lock released, since serializing a 20 MB tree is not something to do while every
/// other command waits for the lock; a failed write is the caller's to report
pub fn save(
    app: &AppHandle,
    game_path: &Path,
...
// Whether the file was written by this very build. One test for both, since a format version only
// changes alongside an app version, and the version that wrote the file is what the page needs to name
// either way
...
// back in: an empty index would look like a built database to every command that asks for one, and the
// page would report zero of everything
...
// The stamps of everything a build of `game_path` reads: the game's own paks and the mod paks. The mod
// list comes from the same two helpers the archive pool is built from (see `AppState::pool`), so a
// fingerprint can only differ when the build's own input did
...
// mid-write then leaves the previous file untouched, instead of a truncated one that reads as corrupt.
// Shared with `settings`, which writes its own small file the same way
...
// A directory as a stable string: lowercased, `/` separators, no trailing one. Windows paths are
// case-insensitive and the frontend hands back the text the user chose, so the same directory can
// arrive spelled differently and normalizing before hashing keeps both spellings on one cache file
...
// FNV-1a over the string's bytes, as 16 lowercase hex digits. Written out rather than taken from
// `std::hash`: a cache file name has to keep meaning the same directory across builds, which a hasher
// free to change its algorithm does not promise
...
// Unix seconds, or 0 when the clock reads before the epoch — a stamp nothing can match, so such a file
