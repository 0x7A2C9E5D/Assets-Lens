...
/// What a build takes out of the state: the resolver to parse with, the directory it parses, and the
/// pool the mods are read through
...
/// The progress bar of one build: the channel it reports through and how much of it each phase owns.
///
/// How many mods there are settles the shape of the bar before any of them is read, and the split is
/// fixed rather than measured: it keeps the bar from jumping back once the game data is done. The
/// count is read off the pool, where a failed lock counts as no mods — mods are optional.
struct Report {
...
/// One pack's slice of the game's share of the bar: the pieces never overlap, so the bar only ever
/// moves forward
#[derive(Clone, Copy)]
struct Slice {
    share: f32,
...
/// The index a build assembles: the merged database with the caches read out of it, and the sources
/// of everything the mods contribute
struct Assembly {
    db: MergedDatabase,
...
/// One build, off the main thread: what it needs is taken from the state, the game's packs and then
/// the mods are parsed and merged over them, and the result is published — the state lock is held only
/// at the two ends (see `build_inputs` and `publish`).
fn build(
    app: &AppHandle,
    state: &SharedState,
    on_progress: &Channel<BuildProgress>,
...
/// command needs that lock to answer. The pool is taken here too, so it is built from the mod
/// directory this build belongs to.
...
fn parse_and_merge(
    ...
    // parsed. This runs before the mods are merged, so it only ever sees game data: mod visuals resolve
    // their own references as they are merged (see `mods::merge_into`).
    ...
    //
    // The parameter names of the virtual texture bindings are *not* read here either: they come from
    // the materials' own templates and are read per detail view (see `ensure_virtual_texture_parameters`).
...
/// Parse the packs of the game's data directory, in maclarian's own order, and answer whether every
...
/// would look unchanged on the next launch and keep the incomplete index alive — the build itself
/// carries on regardless, since a broken pak costs only the resources it holds.
fn parse_game_paks(
    resolver: &GameDataResolver,
    game_path: &Path,
...
/// Report one pak that failed to parse. It costs only the resources it holds: the build carries on
...
/// Merge the mods over the game's own database.
///
/// Mods come last, over a database that already holds everything the game provides: a mod overrides
/// what it redefines, and its visuals routinely bind the game's own materials.
fn merge_mods(
    pool: &Arc<Mutex<Archives>>,
    report: &Report,
...
/// The statistics of a built index, read off the merged maps.
///
/// Read there rather than off `db.stats()`: maclarian keeps the material map to itself, so the mod
/// materials are only ever in the cache the build filled.
...
/// Persist the index while it is still ours to borrow: the launch after this one then reads it back
/// instead of parsing the packs again.
///
/// Written before it is published (a 20 MB file must not be written under the state lock), and only for
/// a build that read everything — an index missing a pak's resources would otherwise outlive the
/// failure it came from. A failed write costs the next launch a scan and nothing else, so it does not
/// fail the build.
...
/// Read the parameter names of `pending` out of the archives.
///
/// Read on its own: the templates come out of the archives, and the pool lock is not taken while the
/// state lock is held.
fn read_parameters(
    pool: &Arc<Mutex<Archives>>,
