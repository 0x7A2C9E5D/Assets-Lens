...
///
/// A GTS level 0 tile grid describes the whole *tile set*, not one page file — a set is sparse, and
/// `Albedo_Normal_Physical_2` claims 512 x 512 tiles = 65536 px, far beyond anything the game stores.
/// What a single page file holds is the bounding box of its own tiles, and that box times a tile's
/// content area is exactly the DDS size the extractor writes out (`infrastructure::export` sizes it
/// the same way), so that is what is reported here.
///
...
// Parse the page file table of one tile set into `sizes`, unless it is already cached there
...
impl GtsHeader {
    // Pixel size of one tile's content area. A tile carries its neighboring pixels in the border on each
    // side; the content area is the texture itself, which is what the extractor writes out
...
// Header fields at their fixed offsets, following maclarian's own readers (`gts::read_header` /
// `gts::read_sections`): little endian throughout, `num_levels` at byte 40, the tile geometry at
// 52..64, the flat tile info table at 68..80 (12 bytes per record), the packed tile ids at 88..100
// (4 bytes per record) and the page file table at 132..144. The fields are read one by one because
// the offsets are the only thing giving them meaning.
...
// The highest resolution one page file stores, in pixels; `None` when it holds no layer 0 tile. The
// levels are picked the way maclarian picks them, since a page file does not have to store level 0
...
// Ceiling on the page file count of one tile set. The game's largest sets hold a few hundred; the cap
// only exists so a corrupt header cannot be turned into a huge allocation.
