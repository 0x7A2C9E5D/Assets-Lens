...
//! only ever walks a merged file, so both are read here. The format is the one maclarian parses —
//! `VisualBank` / `MaterialBank` / `TextureBank` / `VirtualTextureBank` regions, each holding
//! `Resource` nodes — so the field reading below mirrors `maclarian::merged::parser`.
//!
...
//! `MaterialPresetBank` files are presets pointing at visuals and materials the game already has, so
//! indexing them would only add rows with no mesh and no material of their own. And a mod's virtual
//! textures are indexed but not extracted, because the page file lookup searches the game's own archive
//! (see `application::state::vt_pak`).
...
// A texture a mod material binds, read from its `MaterialBank`. The parameter name stays paired with
// the texture GUID here: the name labels the texture row of a visual, while `MaterialInfo` — the shape
// the material cache uses — keeps GUIDs only.
struct TextureParam {
...
// One visual of a `VisualBank` region, or `None` when it names no id or no mesh source: without either
// of these the visual could neither be shown nor previewed, which is where maclarian draws the same line
